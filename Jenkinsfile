node("ci-node") {
  ...
  stage("checkout") {
    checkout scmGit(branches: [[name: branchName]], extensions: [], userRemoteConfigs: [[url: 'https://github.com/mchekini-check-consulting/staff-manager-ui']])
...
//
  ...
  stage("push docker image") {
    ...
    withCredentials([usernamePassword(credentialsId: 'mchekini', usernameVariable: 'username',
      ...
      sh "sudo docker tag staff-manager-ui mchekini/staff-manager-ui:$branchName"
      sh "sudo docker push mchekini/staff-manager-ui:$branchName"
      sh "sudo docker rmi mchekini/staff-manager-ui:$branchName"
